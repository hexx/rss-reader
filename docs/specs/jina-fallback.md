...
- 429・タイムアウト・5xx の退避。相手の不調は既存の律速・クールダウンの領域（sync-egress-politeness.md）に残す。
...
- トリガーは **403 / 451 のみ**。429・タイムアウト・ボディ超過・5xx では退避しない。
...
| 項目 | 値 |
| --- | --- |
| Jina Fallback の発火条件 | リーダ UA・ブラウザ UA とも 403/451（記事本文のみ） |
| `jina.ai` 枠の枠内最小間隔 | 3 秒 + ジッター（3〜4 秒） |
| Jina リクエストのタイムアウト / サイズ上限 | 15 秒 / 2 MB（本文取得と共通の定数） |
