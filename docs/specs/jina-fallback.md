...
- **短文応答の本文化（未対策）**: Jina が CAPTCHA・人間確認・エラーページを 200 で返すと、その短文がそのまま本文として保存され、記事は本文補完（ADR-0014）の巡回対象から外れて**要約汚染が恒久化**しうる（空本文より悪い）。現状 405/CAPTCHA 型（完結した応答）は Jina を呼ばないため発火しないが、直接取得のタイムアウト起因で起こり得る。長さによる品質ゲートを入れるかは未決（issues/issue-202609010458-content-backfill.md の「引き継ぎ B」）。検知は content-gap-audit.md の「疑い」（200 文字未満）に頼る。
