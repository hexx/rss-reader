...
- クールダウンの更新は DB 側で `max(cooldown_until, 要求値)`・`min(consecutive_throttles + 1, 段階数)` とするため、**並行実行が同時に 429 を記録してもバックオフは戻ることも短い値で上書きされることもない**。ただし段階（30分→60分→90分）の選択は読み取り基準のままなので、並行時に 1 段階遅れることがある。完全な直列化は同期の排他ロック（ADR-0002 で却下済み）を必要とするため、意図的に許容する。
- 登録可能ドメインの解決は公開サフィックスリストの完全な写しではなく、日本中心の短縮リストを使う。未収録の multi-label suffix（例 `co.za`）は本来より細かく区別されるだけで、相手を叩く量は増えない（狂う方向は「必要以上に同一枠へ束ねる」側のみ）。
