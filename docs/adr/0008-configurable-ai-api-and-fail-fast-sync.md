...
> **再検討（2026-09-08）**: OpenAI クレジット切れのような継続型障害で同期が止まり続ける問題を検討した（[ADR-0017](./0017-keep-fail-fast-and-abort-on-write-failure.md)）。縮退運転（要約なし保存）は採らず、記事要約の fail-fast は**維持**。射程を 2 点変更した — Hatena Summary の生成失敗は中断対象から外した（NULL が補完巡回で回収されるため）、記事保存の失敗を同期中断の対象に追加した。
