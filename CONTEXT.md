...
**Hatena Summary（反応要約）**:
はてなブックマークのコメント群から生成する、世間の反応・意見の日本語要約（表示用 HTML スニペット）。コメント0件では生成しない。未生成（NULL）は補完巡回で拾われるため、生成失敗は恒久欠損ではなく一時的遅延。Article Summary とは扱いが違う。
_Avoid_: Reaction Summary, 要約（単独では Article Summary と混同）
...
相手の都合で今回の取得ができなかったが、将来は成功する見込みの**取得**失敗。同期中断（Sync Abort）を引き起こさず、後続の同期で自動的に回復する。AI 生成の失敗と保存の失敗（いずれも同期全体を停止する）とは区別する。
_Avoid_: 同期失敗（AI エラーと混用している呼び名）, soft error

**Ingest Failure（記事取り込み失敗）**:
新着記事の保存が失敗すること。取得の成否（Transient Sync Failure）とは区別する、保存の成否の概念。失敗した記事は保存されないため次の同期で再取り込みされるが、run を止めない自己回復にはしない — 想定内の同時実行競合を除き、保存失敗は同期中断を引き起こす（ADR-0017）。
_Avoid_: 同期失敗（取得・要約・保存を区別しない曖昧な語）, 書き込みエラー（実装の語彙）

**Sync Abort（同期中断）**:
AI 生成の失敗、または記事保存の失敗によって、実行中の同期 run 全体を停止させること。相手の都合による取得失敗（Transient Sync Failure）は同期中断を引き起こさない。中断した run のコミット済みデータは残る（ロールバックしない）。
_Avoid_: fail-fast（方針を表す実装語彙）, run 失敗・cron 失敗（観測手段であり状態の名前ではない）, 同期失敗（取得・要約・保存を区別しない曖昧な語）
...
**新着記事**が公開されてから要約付きで表示されるまでに許容する遅延の合意。通常は現行カデンサ ±10 分、律速時は最大 90 分（枠が空かず Carry-over された場合を「律速時」に含む）。既存記事への Backfill の遅れは対象外。同期中断（Sync Abort）中は適用を停止する — 障害中に新着が増えないこと自体が期待挙動で、遅延に上限を設けない（ADR-0017）。
_Avoid_: 鮮度（数値なしでは合意にならない語）, SLA（外部との約束と誤読される）
