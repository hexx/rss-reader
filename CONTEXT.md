...
**Content Backfill（本文補完）**:
Backfill（はてブ補完）の対概念。Content Gap の既存記事の本文と要約を回復する補完巡回（ADR-0014）。
_Avoid_: Backfill（はてブ補完と混同）, 再取得（単発の取得やり直しと混同）

**Give-up（回収断念）**:
本文補完（Content Backfill）が記事単位で断念した状態。404/410 の即断念と、その他の失敗 5 回での断念がある（ADR-0015）。断念記事は巡回対象から外れ、手動で復活させるまで再取得しない。
_Avoid_: 削除済み・404（断念の原因の一例であり、状態の名前ではない）, 恒久欠損（サイト単位と記事単位を混ぜる曖昧な語）
